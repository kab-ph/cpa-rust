...
use cpa::tools::{read_array_2_from_npy_file, write_array, progress_bar};
...
fn cpa()
{
    ...
    let mut cpa: Cpa = (0..nfiles).into_iter().progress_with(progress_bar(nfiles as usize)).
    map(|n| {
    ...
    write_array("examples/results/corr.npy", cpa.pass_corr_array().view());
